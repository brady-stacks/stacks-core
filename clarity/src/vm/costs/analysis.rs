...
/// Extract the list size multiplier from a list node (for map/filter/fold operations)
fn extract_list_multiplier(list_node: &CostAnalysisNode) -> u64 {
    match &list_node.expr {
            ...
            SymbolicExpressionType::List(list) => {
                    ...
                    .unwrap_or(false)
                {
                    match list[1].clone().expr {
                        SymbolicExpressionType::LiteralValue(value) => match value {
        ...
        match native_function {
            NativeFunctions::Map | NativeFunctions::Filter | NativeFunctions::Fold => {
                if node.children.len() > 1 {
                    ...
                    let multiplier = extract_list_multiplier(list_node);
